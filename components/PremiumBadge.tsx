    ...
    >
      {isPremium ? (
        ...
        <div className="relative group">
          <div className="bg-gray-100 rounded-full p-2 border-2 border-orange-500/50">
            <Crown className="w-5 h-5 text-orange-400" />
          </div>
          
          {/* Tooltip */}
          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
            <div className="bg-gray-100 border border-gray-200 rounded-lg p-3 whitespace-nowrap shadow-xl">
...
export function PremiumCard({ isPremium, onUpgrade }: { isPremium: boolean; onUpgrade: () => void }) {
      ...
      >
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-text-tertiary">Lifetime access unlocked</p>
          </div>
        </div>
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="bg-gray-100/50 border border-gray-200 rounded-xl p-6 hover:border-orange-500/50 transition-colors"
    >
      <div className="flex items-center gap-3 mb-4">
        <div className="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center">
          <Crown className="w-6 h-6 text-text-tertiary" />
        </div>
        <div>
          <h3 className="text-xl font-bold">Upgrade to Premium</h3>
          <p className="text-sm text-text-tertiary">Hold 10,000 BLAZE to unlock</p>
        </div>
      </div>
